...
class Conv1d(torch.nn.Module):
    def __init__(self,
        ...
        resample_filter=[1], fused_resample=False, init_mode='kaiming_normal', init_weight=1, init_bias=0,
    ):
        ...
        f= torch.ones([len(resample_filter)], dtype=torch.float32)
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            ...
            if self.up:
                ...
                x = torch.nn.functional.interpolate(x, scale_factor=[2],mode='linear')  # for stability in kernel size, but slower
            if self.down:
                ...
                x = torch.nn.functional.interpolate(x, scale_factor=[0.5],
                                                    mode='linear')  # for stability in kernel size, but slower
            if w is not None:
...
class UNetBlock1D(torch.nn.Module):
    def __init__(self,
        in_channels, out_channels, emb_channels, up=False, down=False, attention=False,
        num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5,
        resample_filter=[1], resample_proj=False, adaptive_scale=True,
        init=dict(), init_zero=dict(init_weight=0), init_attn=None,
    ):
        super().__init__()
        self.in_channels = in_channels
    ...
    def forward(self, x, emb):
        ...
        return x
@persistence.persistent_class
class UNetBlock1D_natural(torch.nn.Module):
    def __init__(self,
        in_channels, out_channels, emb_channels, up=False, down=False, attention=False,
        num_heads=None, channels_per_head=64, dropout=0, skip_scale=1, eps=1e-5,
        resample_filter=[1], resample_proj=False, adaptive_scale=True,
...
@persistence.persistent_class
class SongUNet1D(torch.nn.Module):
    def __init__(self,
                 img_resolution,  # Image resolution at input/output.
                 in_channels,  # Number of color channels at input.
                 out_channels,  # Number of color channels at output.
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                if implicit_mlp:
                    self.enc[f'{res}x{res}_conv'] = torch.nn.Sequential(
                        Conv1d(in_channels=cin, out_channels=cout, kernel=1, **init),
                        torch.nn.SiLU(),
                        Conv1d(in_channels=cout, out_channels=cout, kernel=1, **init),
                        torch.nn.SiLU(),
                        Conv1d(in_channels=cout, out_channels=cout, kernel=1, **init),
                        torch.nn.SiLU(),
                        Conv1d(in_channels=cout, out_channels=cout, kernel=3, **init),
                    )
                    self.enc[f'{res}x{res}_conv'].out_channels = cout
                else:
                    self.enc[f'{res}x{res}_conv'] = Conv1d(in_channels=cin, out_channels=cout, kernel=3, **init)
            else:
                self.enc[f'{res}x{res}_down'] = UNetBlock1D(in_channels=cout, out_channels=cout, down=True,
                                                          **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res}x{res}_aux_down'] = Conv1d(in_channels=caux, out_channels=caux, kernel=0, down=True,
                                                               resample_filter=resample_filter)
                    self.enc[f'{res}x{res}_aux_skip'] = Conv1d(in_channels=caux, out_channels=cout, kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res}x{res}_aux_residual'] = Conv1d(in_channels=caux, out_channels=cout, kernel=3,
                                                                   down=True, resample_filter=resample_filter,
                                                                   fused_resample=True, **init)
                    caux = cout
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = (res in attn_resolutions)
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock1D(in_channels=cin, out_channels=cout, attention=attn,
                                                                **block_kwargs)
        skips = [block.out_channels for name, block in self.enc.items() if 'aux' not in name]
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = img_resolution >> level
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock1D(in_channels=cout, out_channels=cout, attention=True,
                                                         **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock1D(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock1D(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res in attn_resolutions)
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock1D(in_channels=cin, out_channels=cout, attention=attn,
                                                                **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res}x{res}_aux_up'] = Conv1d(in_channels=out_channels, out_channels=out_channels,
                                                             kernel=0, up=True, resample_filter=resample_filter)
                self.dec[f'{res}x{res}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res}x{res}_aux_conv'] = Conv1d(in_channels=cout, out_channels=out_channels, kernel=3,
                                                           **init_zero)
    ...
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f'{res}x{res}_down'] = UNetBlock1D_natural(in_channels=cout, out_channels=cout, down=True,
                                                          **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res}x{res}_aux_down'] = Conv1d(in_channels=caux, out_channels=caux, kernel=0, down=True,
                                                               resample_filter=resample_filter)
            ...
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = (res in attn_resolutions)
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock1D_natural(in_channels=cin, out_channels=cout, attention=attn,
                                                                **block_kwargs)
        skips = [block.out_channels for name, block in self.enc.items() if 'aux' not in name]
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = img_resolution >> level
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock1D_natural(in_channels=cout, out_channels=cout, attention=True,
                                                         **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock1D_natural(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock1D_natural(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res in attn_resolutions)
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock1D_natural(in_channels=cin, out_channels=cout, attention=attn,
                                                                **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res}x{res}_aux_up'] = Conv1d(in_channels=out_channels, out_channels=out_channels,
    ...
    def forward(self, x):
        ...
        for name, block in self.enc.items():
            ...
            if 'aux_down' in name:
                ...
                x = block(x) if isinstance(block, UNetBlock1D_natural) else block(x)
                #print(f"{name} has output size {x.shape}")
                skips.append(x)
...
@persistence.persistent_class
class EDMPrecond_1D(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        out_channels    = None,
    ...
    def forward(self, x, sigma, imagecondition=None, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        ...
        x_in = torch.cat([c_in * x, imagecondition], dim=1)
...
@persistence.persistent_class
class Precond_1D(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        out_channels    = None,
