...
from training import training_loop_residual
...
data_dir ="/external/2_Datasets/11_TimeGraph/brain/cortical_thickness.txt"
results_dir = f"/external/1_Codes/5_timediff/sEDM/results_residual/"

@click.command()

#SY 3D options
@click.option('--dim3d',      help='Dimensionality of in-outs', metavar='INT',           type=click.IntRange(min=1), default=1, show_default=True)
...
@click.option('--precond',       help='Preconditioning & loss function', metavar='vp|ve|edm',       type=click.Choice(['vp', 've', 'edm', 'pedm','edm1d']), default='edm1d', show_default=True)
...
@click.option('--snap',          help='How often to save snapshots', metavar='TICKS',               type=click.IntRange(min=1), default=1000, show_default=True)
...
def main(**kwargs):
    ...
    c = dnnlib.EasyDict()
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.CustomDataset_both_time', path=opts.data)
    ...
    if opts.dim3d ==2 :
        ...
        if opts.arch == 'ddpmpp':
            ...
            c.network_kwargs.update(model_type='SongUNet1D', embedding_type='fourier', encoder_type='residual', decoder_type='standard')
            c.network_kwargs.update(channel_mult_noise=2, resample_filter=[1,3,3,1], model_channels=128, channel_mult=[2,2,2])
        else:
            assert opts.arch == 'adm'
            c.network_kwargs.update(model_type='DhariwalUNet3D', model_channels=192, channel_mult=[1,2,3,4])
    ...
    if opts.precond == 'vp':
        ...
        assert opts.precond == 'edm1d'
        c.network_kwargs.class_name = 'training.networks.EDMPrecond_1D'
        c.loss_kwargs.class_name = 'training.loss.EDMLoss_1D_Conditional'
    ...
    if use_cuda:
        ...
        training_loop_residual.training_loop(**c)
    else:
